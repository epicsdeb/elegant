...
\author{Program Version 22.1\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 22.1}

Here is a summary of what's changed since release 22.0.  Some
significant improvements were made for modeling of laser/undulator
beam heaters (\verb|LSRMDLTR|) and intrabeam scattering.  It is also
possible now to compute dynamic aperture using parallel resources.
For details and references, see the specific entries in the manual.

\subsection{Bug Fixes for Elements}

\begin{itemize}
\item The \verb|LSRMDLTR| was not correctly including vertical focusing of the undulator when the exact and leading-terms
  expansions were used.  This was fixed.  D. Xiang (SLAC) reported this problem.
\item \verb|UKICKMAP| and \verb|BMAPXY| had a bug in the interpolation routines that resulted in extrapolation being performed
  in some cases.   This might change results with coarse grids.
\item \verb|WATCH| elements may now be placed between identical bending magnets without messing up the 
  transport (previously, unwanted edge effects were added).
\item The radiation matrix (used by \verb|moments_output|) was wrong for \verb|SBEN| when the element was tilted.
\item Fixed some problems with keeping track of whether radiation integrals were computed. This prevents incorrect warnings
  about \verb|SREFFECTS| not being set up.
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item Fixed a bug in adding errors: in previous versions, if elements had errors from
a parameters file and if the user also tried to add differential errors, the
elements would all start from the value of the first occurence of the element.  I.e., the
initial individual values were not used.  
\item Fixed a bug that resulting in quoted string parameters being converted to upper case in
  elements inserted with \verb|insert_elements|.
\item The magnet-profile output (requested via \verb|run_setup|) now includes \verb|KQUSE| elements.
\item The corrected emittance computations (from the \verb|sigma| file from \verb|run_setup|) were incorrect
  when the energy spread was zero.
\end{itemize}

\subsection{New and Modified Elements}
\begin{itemize}
\item The default integration order for \verb|CSBEND| and \verb|CSRCSBEND| was changed from 2 to 4, to resolve
  a common source of perplexing results from users.
\item The \verb|IBSCATTER| element can now compute intrabeam scattering in a sliced fashion, which is useful
  for linac simulation.  This was done by A. Xiao (ANL).
\item The \verb|LSRMDLTR| element now supports a laser time profile, synchrotron radiation, higher-order laser modes
  (up to TEM44), and allows the user to offset the laser beam in x, y, and z.  This was
  requested by D. Xiang (SLAC).  Thanks to P. Emma (SLAC) for providing an very helpful relevant article.
  The element is also parallel-capable now.
\item \verb|CSRCSBEND| and \verb|TWLA| elements now participate in beam moments computations (i.e., \verb|moments_output|).
\item Added tracking-based computation of the linear matrix for \verb|TWLA|, \verb|TWMTA|, and \verb|MAP_SOLENOID|, so that
  twiss computations and other matrix-based computations will properly include these elements.  Tracking is unchanged.
  S. Reiche (PSI) pointed out this deficiency.
\item \verb|MARK| elements with \verb|FITPOINT=1| now store parameters named \verb|betaxBeam| and \verb|alphaxBeam|,
  along with the same quantities for the y plane.  These are the twiss parameters computed from the beam.  They
  also appear in the sigma output file (specified by \verb|run_setup|).
  This was requested by S. Reiche (PSI).
\item Added the \verb|DELTAP| parameter to the \verb|EMATRIX| element, allowing one to specify an energy change as
  part of an explicit matrix.  Requested by Y. Roblin (TJNAF).
\item The files created by the \verb|WATCH| element now contain a parameter with the name of the upstream element, which
  is helpful in making plots.
\item The \verb|XYFACTOR| element was added to \verb|UKICKMAP|, to allow expanding the kickmap size.
\item \verb|RFCA| elements may now be subdivided using the regular element-division controls (\verb|run_setup| or
  \verb|divide_elements|).
\item The \verb|EMITTANCE| element was added.  It allows transforming a beam during tracking to force the emittances
  to specified values.
\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item The \verb|rpn_load| command can now load the parameters from an SDDS file instead of the columns.
  This is obtained by setting \verb|load_parameters=1|.
\item Aperture determination using \verb|find_aperture| and the line-search technique was parallelized.  This was done by
  Y. Wang (ANL).
\item The aperture determined by the \verb|find_aperture| command may now be optimized.  (However, this can't be done in the
  parallel version at present.)
\item \verb|alter_elements| now changes the reference value for the element. This means that errors will be added relative to
  the altered value.
\item The \verb|optimization_term| command can now read optimization terms from an SDDS file.
\end{itemize}

\subsection{Changes to Related Programs and Files}

\begin{itemize}
\item A bug was fixed in the {\tt haissinski}.  The \verb|-wake| option was nonfunctional due to an argument parsing
  problem.  This was reported by A. Rusanov (NSRRC).
\item The script {\tt astra2elegant} was replaced by a compiled program.  The program is not only much faster, but
  also has corrections for the conversion that will be evident in long bunches or low energy beams.  Thanks to 
  P. Piot (NIU) and J. Rowland (DLS) for input.
\item The scripts {\tt impact2sdds} and {\tt impact2elegant} were added.  These provide translation of IMPACT-T \cite{IMPACT} output
  into SDDS files for more convenient postprocessing, as well as translation of IMPACT-T phase space into {\tt elegant}'s
  conventions.  
\item The scripts {\tt track2sdds}, {\tt track2mag}, and {\tt elegant2track} were added.  These provide a interface between
  the ASCII version of TRACK and SDDS/{\tt elegant}.
\end{itemize}
